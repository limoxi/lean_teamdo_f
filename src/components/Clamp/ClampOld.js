...
export default {
  ...
  methods: {
    addListener (target, handler) {
      if (target.addEventListener) {
        target.addEventListener('clamp', handler);
      } else if (target.attachEvent) {
        target.attachEvent('onclamp', handler);
      } else {
        target['onclamp'] = handler;
      }
    },

    removeListener (target, handler) {
      if (target.removeEventListener) {
        target.removeEventListener('clamp', handler);
      } else if (target.detachEvent) {
        target.detachEvent('onclamp', handler);
      } else {
        target['onclamp'] = handler;
      }
    },
    ...
    init () {
      ...
      if (this.autoresize) {
        ...
        this.addListener(this.$el, resizeCallback)
        this.unregisterResizeCallback = () => {
          this.removeListener(this.$el, resizeCallback)
        }
      }
      this.update()
    },
    ...
    getText () {
      ...
      return content ? content.text.replace(/<(?!img).*?>/g, '') : ''
    },
    moveEdge (steps) {
